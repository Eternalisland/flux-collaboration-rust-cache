...
impl HighPerfMmapStorage {
    ...
    fn try_shrink_tail(&self) -> std::io::Result<()> {
        ...
        if reclaimed > 0 {
            ...
            // 2. 【关键】使用临时的 None 替换读映射，强制释放所有读者
            // 这会导致短暂的读取不可用，但这是 Windows 上唯一的办法
            let old_mmap = self.mmap_ro.swap(Arc::new(None));
            drop(old_mmap);  // 显式释放旧映射的引用

            // 3. 等待所有读者释放引用（通过短暂自旋）
            // 注意：这里有一个小的时间窗口，读请求会失败并重试
    ...
    fn compact_data_file(&self, valid_entries: &HashMap<String, (u64, u64)>) -> std::io::Result<u64> {
        ...
        // 【Windows 兼容】在读取前先获取数据，不持有 mmap 引用
        ...
        // 【关键】强制释放读映射
        let old_ro = self.mmap_ro.swap(Arc::new(None));
        drop(old_ro);
